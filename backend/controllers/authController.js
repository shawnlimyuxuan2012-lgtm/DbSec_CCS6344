...
const register = async (req, res) => {
  ...
  if (!pdpa_consent)
    return res
      .status(400)
      .json({ message: "PDPA consent is required to register" });
  ...
  try {
    const pool = await getPool("admin"); // ← admin pool for INSERT
...
const login = async (req, res) => {
  ...
  try {
    const pool = await getPool("student"); // ← student pool is fine for sp_AuthenticateUser
    const result = await pool
      .request()
      .input("email", sql.NVarChar, email)
...
const forgotPassword = async (req, res) => {
  ...
  try {
    const pool = await getPool("admin"); // ← admin pool for INSERT into password_reset_tokens
    const result = await pool
      .request()
      .input("email", sql.NVarChar, email)
      .query(
        "SELECT id, name FROM users WHERE email = @email AND is_deleted = 0",
      );
...
const resetPassword = async (req, res) => {
  ...
  try {
    const pool = await getPool("admin"); // ← admin pool for UPDATE users
